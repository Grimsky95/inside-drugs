cfg.Prefix.RegisterServerCallback('SellDrugs:Create_Wholesale', function(source, cb, price, amount)
    local src = source
...
cfg.Prefix.RegisterServerCallback('SellDrugs:Wholesale_Trade', function(source, cb)
    local src = source
    local xPlayer = cfg.Prefix.GetPlayerFromId(src)
    ...
    if item and item.amount >= Player["id"..src].wholesale.Amount then
        ...
        local full_name = xPlayer.variables.firstName.." "..xPlayer.variables.lastName
        Send_Discord_Log(3447003, translate("server", "discord_wholesale_sell_title", GetPlayerName(src), src, xPlayer.identifier), "inside-scripts (Advanced Drug Sales 💊)", translate("server", "discord_wholesale_sell_message", full_name, reformatInt(Player["id"..src].wholesale.Amount), item.label, reformatInt(Price)), "Support - discord.gg/URKNMSwx5W")
        cb(true)
...
RegisterNetEvent("SellDrugs:Wholesale_Attack", function(scenario)
    ...
    if scenario == "ped_dead" then
        ...
        local full_name = xPlayer.variables.firstName.." "..xPlayer.variables.lastName
        Send_Discord_Log(3447003, translate("server", "discrod_wholesale_attack_title", GetPlayerName(src), src, xPlayer.identifier), "inside-scripts (Advanced Drug Sales 💊)", translate("server", "discrod_wholesale_attack_message_player_dead", full_name, reformatInt(CurrentMoney)), "Support - discord.gg/URKNMSwx5W")
    elseif scenario == "enemy_dead" then
        ...
        local full_name = xPlayer.variables.firstName.." "..xPlayer.variables.lastName
        Send_Discord_Log(3447003, translate("server", "discrod_wholesale_attack_title", GetPlayerName(src), src, xPlayer.identifier), "inside-scripts (Advanced Drug Sales 💊)", translate("server", "discrod_wholesale_attack_message_enemy_dead", full_name, reformatInt(configPrice)), "Support - discord.gg/URKNMSwx5W")
    end
end)