RegisterNetEvent("SellDrugs:Status_Selling_Drugs", function(src)
    ...
    local citizenID = cfg.Prefix.GetPlayerFromId(source).identifier

    if Player["id"..source].sellingDrugs then
        TriggerClientEvent("SellDrugs:Stop", source)
        Player["id"..source].sellingDrugs = false
        serverNotify(source, translate("server", "notify_retail_stop"), "info", 5000)
    else
        local Drugs = {}
        ...
        if Drugs_Amount > 0 then
            ...
            serverNotify(source, translate("server", "notify_retail_start"), "info", 5000)
        else
...
cfg.Prefix.RegisterServerCallback('SellDrugs:Start_Interaction', function(source, cb)
    local src = source
    local Drugs = {}
    local Drugs_Amount = 0
...
RegisterNetEvent("SellDrugs:Make_Trade", function(Drug, Amount)
    ...
    if #Player["id"..src].Prices == 0 or not item then
        ...
        Send_Discord_Log(3447003, translate("server", "discord_hacker_title", GetPlayerName(src), src, xPlayer.identifier), "inside-scripts (Advanced Drug Sales 💊)", translate("server", "discord_hacker_make_trade_message", full_name), "Support - discord.gg/URKNMSwx5W")
        return
    end
    ...
    if item and item.amount >= Amount then
        ...
        Player["id"..src].Prices = {}
        local full_name = xPlayer.variables.firstName.." "..xPlayer.variables.lastName
        Send_Discord_Log(3447003, translate("server", "discord_retail_sell_title", GetPlayerName(src), src, xPlayer.identifier), "inside-scripts (Advanced Drug Sales 💊)", translate("server", "discord_retail_sell_message", full_name, reformatInt(Amount), item.label, reformatInt(Price)), "Support - discord.gg/URKNMSwx5W")
    end
end)
