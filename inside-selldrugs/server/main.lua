cfg.Prefix = exports[cfg.ExportPrefix]:getSharedObject()

cfg.Prefix.RegisterServerCallback('SellDrugs:Get_Player_Info', function(source, cb)
    local src = source
...
cfg.Prefix.RegisterServerCallback('SellDrugs:Calculate_Percentage', function(source, cb, itemName, price, isWholesale)
    local src = source
    ...
    if not itemConfig then
        ...
        Send_Discord_Log(3447003, translate("server", "discord_hacker_title", GetPlayerName(src), src, xPlayer.identifier), "inside-scripts (Advanced Drug Sales 💊)", translate("server", "discord_hacker_calculate_percentage_message", full_name, itemName, price, mode), "Support - discord.gg/URKNMSwx5W")
        return
    end
    ...
    if not isWholesale then
        ...
        if HigherPrice then serverNotify(src, translate("server", "notify_retail_negotiating"), "info", 5000) return end
    end
...
RegisterNetEvent("SellDrugs:Player_Joined", function()
    local src = source
    local citizenID = cfg.Prefix.GetPlayerFromId(src).identifier
