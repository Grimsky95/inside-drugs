...
function togglePhoneNUI(showNUI)
    cfg.Prefix.TriggerServerCallback('SellDrugs:Get_Player_Info', function(data)
...
RegisterNUICallback('dialogBack', function(data, cb)
    ...
    elseif data.action == "sell" then
        cfg.Prefix.TriggerServerCallback('SellDrugs:Calculate_Percentage', function(result, totalNegotiation)
...
RegisterNUICallback('phoneStartTradeMission', function(data, cb)
    cfg.Prefix.TriggerServerCallback('SellDrugs:Create_Wholesale', function(result)
        if result then
